...
const mappingString = `
{
	"serviceconfighistory": {
		"properties": {
			...
			"CommitMessage":	{"type": "string", "index": "not_analyzed"},
			"ConfigFiles": {
				"properties": {
...
var (
	// MAPPING is the elastic mapping for service config history
	MAPPING, mappingError = elastic.NewMapping(mappingString)
)

func init() {
	if mappingError != nil {
		glog.Fatalf("error creating service config history mapping %s: %s", mappingString, mappingError)
	}
}
